...
def write_a_data(action_list, filename=None):
    ...
    trajectory_data = {
        ...
        "trajectory_length": (len(action_list)-1)//2
    }
    # filename = "20250917_031017_webarena_results.jsonl"
...
def Get_multi_turn_response(question, answer):
    ...
    # url = init_url
    history_info = "\n"
    action_list = [{"role":"system","content":system_prompt}]
    is_error = False
    error_msg = ""
    ...
    try:
        ...
        for i in range(6):
            ...
            action_list.append({"role":"user","content":real_prompt})
            
            try:
                # response = get_response(prompt, temperature=0)
                response = get_response(action_list, temperature=0)
                ...
                action_list.append({"role":"assitant","content":response})
